...
export function ProgressPage() {
  const { plan, completedCount, totalCount } = useTraining();

  // Calculate weekly stats
  const weeklyData = plan?.weeks.map((week, index) => {
      ...
      <div className="p-6 lg:p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-card rounded-2xl border border-border p-6">
            ...
            <Progress value={overallProgress} className="h-1.5 mt-2" indicatorClassName="bg-hero-gradient" />
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border p-6">
            ...
            <div className="text-3xl font-display font-bold">
              3 <span className="text-lg text-muted-foreground font-normal">days</span>
            </div>
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <Calendar className="w-4 h-4 text-bike" />
            </div>
            <div className="text-xl font-display font-bold">
              {plan?.phase || 'Base Building'}
            </div>
          </div>
        </div>
                ...
                {plan?.weeks.map((week, index) => {
                  ...
                  return (
                    <tr key={index} className="border-b border-border/50">
                      ...
                      <td className="py-4 px-4">{week.totalHours}h</td>
                      <td className="py-4 px-4">{runDistance}km</td>
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <Progress value={progress} className="h-2 w-24" indicatorClassName="bg-hero-gradient" />
